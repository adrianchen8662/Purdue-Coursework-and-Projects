...
int countWord(char * filename, char * word, char * line, int size)
{
  ...
  int counter = 0;
  int wordcounter;
  //Open the file. If the file does not exist, return false
  fp = fopen(filename, "r");
  if(fp == NULL)
  {
    return -1;
  ...
  //If the file does exist, set sum = 0 and continue
  wordcounter = 0;
  //Loop until it reaches the end of the file(EOF)
  while(!feof(fp))
  {
    fseek(fp,counter,SEEK_SET);
    fgets(line,strlen(word) + 1,fp);
